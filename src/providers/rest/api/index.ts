...
export const getFileComponent = (fileId: string, accessToken: string) => {
  counter++;
  return figmaRestApi.get<FileComponentsResponse>(
...
export const getFileNodes = (
  ...
  counter++;
  return figmaRestApi.get<FileNodesResponse>(
    "files/" + fileId + "/nodes?ids=" + ids.join(","),
    {
      headers: {
        ...getFigmaAuthHeaders(accessToken),
      },
...
export const getAssetURL = (
  ...
  counter++;
  return figmaRestApi.get<FileImageResponse>(
    "images/" + fileId + "/?ids=" + ids.join(",") + "&format=" + extension,
    {
      headers: {
        ...getFigmaAuthHeaders(accessToken),
      },
...
export const getComponentSets = (fileId: string, accessToken: string) => {
  counter++;
  return figmaRestApi.get<FileComponentSetsResponse>(
    "files/" + fileId + "/component_sets",
    {
      headers: {
        ...getFigmaAuthHeaders(accessToken),
      },
...
export const getComponentSetNodes = (
  ...
  counter++;
  return figmaRestApi.get<FileNodesResponse>(
    "files/" + fileId + "/nodes?ids=" + ids.join(",") + "&plugin_data=shared",
    {
      headers: {
        ...getFigmaAuthHeaders(accessToken),
      },
